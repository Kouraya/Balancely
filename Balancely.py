                ...
                with fc_col_r:
                    ...
                    if goal_fc>0:
                        ...
                        goal_html = (
                            ...
                            f"<div style='font-family:DM Mono,monospace;color:#475569;font-size:11px;'>{'Puffer: +' if diff_goal>=0 else 'Fehlbetrag: '}{abs(diff_goal):,.2f} {_currency_sym}</div></div></div>")
                    st.markdown(
                        f"<div style='background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(148,163,184,0.08);border-radius:16px;padding:20px 22px;'>"
    ...
    # ============================================================
        ...
        if active_tab == "Profil":
            ...
            try:
                ...
                st.markdown(f"<div style='background:rgba(10,16,30,0.5);border:1px solid rgba(148,163,184,0.06);border-radius:10px;padding:12px 16px;display:inline-block;margin-bottom:12px;'><span style='font-family:DM Mono,monospace;color:{_t['primary']};font-size:15px;font-weight:500;'>@{user_name}</span></div>", unsafe_allow_html=True)
                if not can_change:
                    ...
                    with st.form("username_change_form"):
                        ...
                        if st.form_submit_button("Benutzername ändern", use_container_width=True, type="primary"):
                            ...
                            if len(new_uname) < 3: st.error("❌ Mindestens 3 Zeichen erforderlich.")
                                ...
                                for ws,col in [("transactions","user"),("toepfe","user"),("goals","user"),("settings","user")]:
                                    try:
                                        df_ws = _gs_read(ws)
            ...
            last_rename_str = _user_settings.get('last_username_change','')
            can_rename = True; days_until = 0
            if last_rename_str:
                try:
                    last_rename = datetime.date.fromisoformat(str(last_rename_str))
                    delta = (datetime.date.today() - last_rename).days
                    if delta < 30: can_rename = False; days_until = 30 - delta
                except: pass
            if can_rename:
                with st.form("rename_form"):
                    new_uname = st.text_input("Neuer Benutzername", placeholder="Mindestens 3 Zeichen")
                    if st.form_submit_button("Benutzername ändern", use_container_width=True, type="primary"):
                        new_uname = new_uname.strip()
                        if len(new_uname) < 3:
                            st.error("❌ Mindestens 3 Zeichen erforderlich.")
                        elif new_uname == user_name:
                            st.error("❌ Das ist bereits dein Benutzername.")
                        else:
                            df_u_r = _gs_read("users")
                            if new_uname in df_u_r['username'].values:
                                st.error("❌ Benutzername bereits vergeben.")
                            else:
                                df_u_r.loc[df_u_r['username']==user_name,'username'] = new_uname
                                _gs_update("users", df_u_r)
                                # Alle verknüpften Daten umbenennen
                                ...
                                save_user_settings(new_uname, last_username_change=str(datetime.date.today()))
                                st.session_state['user_name'] = new_uname
                                for k in [k for k in st.session_state if k.startswith("_gs_cache_")]: del st.session_state[k]
                                st.success(f"✅ Benutzername auf @{new_uname} geändert!"); st.rerun()
            else:
                st.markdown(f"<div style='background:rgba(248,113,113,0.06);border:1px solid rgba(248,113,113,0.15);border-radius:10px;padding:12px 16px;font-family:DM Sans,sans-serif;color:#94a3b8;font-size:13px;'>🔒 Benutzername kann erst in <span style='color:#f87171;font-weight:500;'>{days_until} Tagen</span> wieder geändert werden.</div>", unsafe_allow_html=True)
                    ...
                    if user_df.empty:
                        ...
                        if search_q:
                            mask_s = (
                                user_df['kategorie'].str.lower().str.contains(search_q, na=False) |
                                user_df['notiz'].astype(str).str.lower().str.contains(search_q, na=False) |
                                user_df['betrag_anzeige'].str.lower().str.contains(search_q, na=False) |
                                user_df['typ'].str.lower().str.contains(search_q, na=False)
                            )
                            user_df = user_df[mask_s]
