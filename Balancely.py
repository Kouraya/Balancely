...
def format_timestamp(ts_str, datum_str) -> str:
    ...
    try:
        ...
        elif ts.date() == today - datetime.timedelta(days=1): return f"gestern {uhr}"
        return ts.strftime("%d.%m.%Y %H:%M")
    except Exception:
        try:
            d = datetime.date.fromisoformat(str(datum_str))
            if d == today:            return "heute"
            elif d == today - datetime.timedelta(days=1): return "gestern"
    ...
    <html><body style="font-family:sans-serif;background:#020617;color:#f1f5f9;padding:40px;">
        <div style="max-width:480px;margin:auto;background:#0f172a;border-radius:16px;
            ...
            <div style="margin:24px 0;padding:20px;background:#1e293b;border-radius:12px;
                        ...
                        letter-spacing:8px;color:#38bdf8;">{code}</div>
...
.sub-title {
    text-align: center; color: #475569; font-size: 15px;
    font-weight: 400; letter-spacing: 0.3px; margin-bottom: 48px; margin-top: 8px;
}
[data-testid="stForm"] {
...
div[data-baseweb="input"], div[data-baseweb="base-input"] {
    ...
    border: 1px solid rgba(148,163,184,0.1) !important; border-radius: 10px !important;
...
input { padding-left: 15px !important; color: #e2e8f0 !important;
        font-family: 'DM Sans', sans-serif !important; font-size: 14px !important; }
input::placeholder { color: #334155 !important; }
div[data-testid="stDateInput"] > div {
    background-color: rgba(15,23,42,0.6) !important;
    border: 1px solid rgba(148,163,184,0.1) !important; border-radius: 10px !important;
    box-shadow: none !important; min-height: 42px !important; overflow: hidden !important;
}
...
[data-testid="stSidebar"] .stMarkdown p {
    font-family: 'DM Sans', sans-serif !important; color: #475569 !important; font-size: 13px !important;
}
[data-testid="stSidebar"] [data-testid="stRadio"] > div > label {
    border: 1px solid transparent !important; border-radius: 10px !important;
    padding: 9px 14px !important; margin-bottom: 3px !important; color: #475569 !important;
    font-family: 'DM Sans', sans-serif !important; font-size: 14px !important;
    font-weight: 400 !important; transition: all 0.15s ease !important; letter-spacing: 0.1px !important;
}
[data-testid="stSidebar"] [data-testid="stRadio"] > div > label:hover {
    border-color: rgba(56,189,248,0.15) !important; color: #94a3b8 !important;
    background: rgba(56,189,248,0.04) !important;
}
[data-testid="stSidebar"] [data-testid="stRadio"] > div > label:has(input:checked) {
    border-color: rgba(56,189,248,0.25) !important; background: rgba(56,189,248,0.08) !important;
...
button, [data-testid="stPopover"] button, div[data-baseweb="select"],
div[data-baseweb="select"] *, div[data-testid="stDateInput"],
...
div[data-testid="stDialog"] > div, div[role="dialog"] {
    ...
    border: 1px solid rgba(148,163,184,0.1) !important;
    border-radius: 18px !important; box-shadow: 0 40px 80px rgba(0,0,0,0.6) !important;
}
...
[data-testid="stPopover"] > div {
    background: #0d1729 !important; border: 1px solid rgba(148,163,184,0.1) !important;
    border-radius: 12px !important; box-shadow: none !important;
}
[data-testid="stExpander"] {
    border: 1px solid rgba(148,163,184,0.08) !important;
    border-radius: 12px !important; background: rgba(10,16,32,0.5) !important;
}
.topf-progress-bg { background: rgba(30,41,59,0.8); border-radius: 99px; height: 5px; overflow: hidden; margin-top: 8px; }
.topf-progress-fill { height: 100%; border-radius: 99px; transition: width 0.6s ease; }
.heatmap-cell {
    display: inline-block; width: 36px; height: 36px; border-radius: 8px;
    text-align: center; line-height: 36px; font-family: 'DM Mono', monospace;
    font-size: 11px; font-weight: 500; transition: transform 0.15s ease; cursor: default;
}
</style>
""", unsafe_allow_html=True)

# ============================================================
#  Konstanten & Session State
# ============================================================
...
_STATE_DEFAULTS = {
    'logged_in': False, 'user_name': "", 'auth_mode': 'login',
    't_type': 'Ausgabe', 'pending_user': {}, 'verify_code': "",
    'verify_expiry': None, 'reset_email': "", 'reset_code': "", 'reset_expiry': None,
    ...
    'analysen_zeitraum': 'Monatlich', 'heatmap_month_offset': 0,
    'topf_edit_idx': None, 'topf_delete_idx': None, 'show_new_topf': False,
    'settings_tab': 'Profil',
...
# ============================================================
#  GSheet Cache-System  (Fix 1: Performance)
# ============================================================

def _gs_read(worksheet: str) -> pd.DataFrame:
    """Cached read — holt Daten nur einmal pro Session/Worksheet."""
    key = f"_gs_cache_{worksheet}"
    if key not in st.session_state:
        st.session_state[key] = conn.read(worksheet=worksheet, ttl=0)
    return st.session_state[key].copy()

def _gs_update(worksheet: str, df: pd.DataFrame):
    """Schreibt ins Sheet und aktualisiert den Cache sofort (write-through)."""
    conn.update(worksheet=worksheet, data=df)
    st.session_state[f"_gs_cache_{worksheet}"] = df.copy()

def _gs_invalidate(*worksheets: str):
    """Löscht den Cache für ein oder mehrere Worksheets."""
    ...
    try:
        ...
        for _, r in df[df['user'] == user].iterrows():
            ...
            result.append({
                ...
                'ziel': float(r.get('ziel', 0) or 0), 'gespart': float(r.get('gespart', 0) or 0),
                'emoji': str(r.get('emoji', '🪣')), 'farbe': str(r.get('farbe', '#38bdf8')),
            })
        return result
    except Exception:
        return []
...
def save_topf(user: str, name: str, ziel: float, emoji: str):
    ...
    new_row = pd.DataFrame([{
        'user': user, 'id': topf_id, 'name': name, 'ziel': ziel, 'gespart': 0,
        'emoji': emoji, 'farbe': TOPF_PALETTE[existing_count % len(TOPF_PALETTE)], 'deleted': '',
    }])
...
def update_topf_gespart(user: str, topf_id: str, topf_name: str, delta: float):
    ...
    try:
        ...
        new_row = pd.DataFrame([{
            "user": user, "datum": str(datetime.date.today()),
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
            "typ": "Spartopf", "kategorie": f"🪣 {topf_name}",
...
def section_header(title: str, subtitle: str = ""):
    sub = (f"<p style='font-family:DM Sans,sans-serif;color:#475569;font-size:13px;"
           f"margin:2px 0 20px 0;'>{subtitle}</p>") if subtitle else "<div style='margin-bottom:20px;'></div>"
    st.markdown(
        f"<p style='font-family:DM Mono,monospace;color:#475569;font-size:10px;"
        f"font-weight:500;letter-spacing:1.5px;text-transform:uppercase;margin:0 0 4px 0;'>"
        f"{title}</p>{sub}", unsafe_allow_html=True)

def inject_theme(t: dict):
    """Fix 2: Alle Button-Typen theme-adaptiert stylen."""
    st.markdown(f"""
    <style>
    [data-testid="stAppViewContainer"] {{
        background:
            radial-gradient(ellipse 80% 50% at 20% -10%, {t['grad1']} 0%, transparent 60%),
    ...
    [data-testid="stSidebar"] {{
        background: linear-gradient(180deg, {t['bg1']} 0%, {t['bg3']} 100%) !important;
    }}
    ...
    [data-testid="baseButton-primary"]:hover {{
        box-shadow: 0 6px 20px {t['primary']}60 !important;
        transform: translateY(-1px) !important; filter: brightness(1.08) !important;
    }}
    button[kind="secondary"], [data-testid="baseButton-secondary"] {{
        background: rgba(255,255,255,0.04) !important;
        border: 1px solid {t['primary']}30 !important; color: {t['primary']} !important;
        transition: all 0.2s ease !important;
    }}
    button[kind="secondary"]:hover, [data-testid="baseButton-secondary"]:hover {{
        background: {t['primary']}15 !important; border-color: {t['primary']}60 !important;
    }}
...
def new_category_dialog():
    ...
    with nc1: new_emoji = st.text_input("Emoji", placeholder="🎵", max_chars=4, help="Windows: **Win + .** | Mac: **Ctrl+Cmd+Space**")
    with nc2: new_name  = st.text_input("Name", placeholder="z.B. Musik")
    nc_typ = st.selectbox("Typ", ["Ausgabe", "Einnahme"], index=0 if typ == "Ausgabe" else 1)
...
def edit_category_dialog():
    ...
    init_emoji, init_name = (parts[0], parts[1]) if len(parts) == 2 and len(parts[0]) <= 4 else ('', old_label)
    st.markdown(f"<p style='color:#64748b;font-size:13px;margin-bottom:16px;font-family:DM Sans,sans-serif;'>Aktuell: <span style='color:#38bdf8;'>{old_label}</span></p>", unsafe_allow_html=True)
    nc1, nc2 = st.columns([1, 3])
    with nc1: new_emoji = st.text_input("Emoji", value=init_emoji, max_chars=4, placeholder="🎵", help="Windows: **Win + .** | Mac: **Ctrl+Cmd+Space**")
    with nc2: new_name  = st.text_input("Name", value=init_name, placeholder="z.B. Musik")
    new_typ = st.selectbox("Typ", ["Ausgabe", "Einnahme"], index=0 if typ == "Ausgabe" else 1)
    col_save, col_cancel = st.columns(2)
    with col_save:
        if st.button("Speichern", use_container_width=True, type="primary"):
            if not new_name.strip():
...
def confirm_delete_cat():
    ...
    if not data:
        st.rerun(); return
    st.markdown(f"<p style='color:#e2e8f0;font-size:15px;margin-bottom:8px;font-family:DM Sans,sans-serif;'>Kategorie wirklich löschen?</p><p style='color:#64748b;font-size:14px;'>{data['label']}</p>", unsafe_allow_html=True)
...
def confirm_delete(row_data):
    st.markdown(
        f"<p style='color:#e2e8f0;font-size:15px;margin-bottom:6px;font-family:DM Sans,sans-serif;'>Eintrag wirklich löschen?</p>"
...
if st.session_state['logged_in']:
    ...
    _currency_sym  = CURRENCY_SYMBOLS.get(_user_settings.get('currency', 'EUR'), '€')  # Fix 3

    inject_theme(_t)  # Fix 2
    ...
    with st.sidebar:
        ...
        if _avatar and _avatar.startswith('http'):
            st.markdown(
                ...
                f"<img src='{_avatar}' style='width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid {_t['primary']}40;'>"
                ...
                f"<div style='font-family:DM Mono,monospace;font-size:10px;color:#334155;'>{_user_settings.get('currency','EUR')} · {_theme_name}</div>"
                f"</div></div>", unsafe_allow_html=True)
        else:
            _initials = st.session_state['user_name'][:2].upper()
            st.markdown(
                f"<div style='display:flex;align-items:center;gap:10px;margin-bottom:20px;'>"
                f"<div style='width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,{_t['accent']},{_t['accent2']});display:flex;align-items:center;justify-content:center;font-family:DM Sans,sans-serif;font-size:13px;font-weight:600;color:#fff;flex-shrink:0;'>{_initials}</div>"
                f"<div><div style='font-family:DM Sans,sans-serif;font-size:13px;color:#e2e8f0;font-weight:500;'>{st.session_state['user_name']}</div>"
                f"<div style='font-family:DM Mono,monospace;font-size:10px;color:#334155;'>{_user_settings.get('currency','EUR')} · {_theme_name}</div>"
                f"</div></div>", unsafe_allow_html=True)
        ...
        if st.button("Logout ➜", use_container_width=True, type="secondary"):
            for _k in [k for k in st.session_state if k.startswith("_gs_cache_")]:
        ...
        try:
            ...
            if "user" not in df_t.columns:
                ...
                if monat_df.empty:
                    ...
                    bank           = ein - aus - dep_monat + spartopf_netto
                    dep_gesamt     = alle[alle["typ"] == "Depot"]["betrag_num"].abs().sum()
                    topf_gesamt_val= sum(t['gespart'] for t in load_toepfe(st.session_state["user_name"]))
                    ...
                    if offset == 0:
                        ...
                        if _budget > 0:
                            ...
                            st.markdown(
                                ...
                                f"<div style='width:{_bpct:.0f}%;height:100%;background:{_bcol};border-radius:99px;transition:width 0.6s ease;'></div></div></div>",
                                unsafe_allow_html=True)

                    # Sparziel-Warnung
                    if offset == 0:
                        _goal_dash = load_goal(st.session_state["user_name"])
                        _sp_einz   = abs(monat_df[(monat_df["typ"]=="Spartopf") & (monat_df["betrag_num"]<0)]["betrag_num"].sum())
                        if _goal_dash > 0:
                            _effektiv  = bank + _sp_einz
                            _th        = f" · davon {_sp_einz:,.2f} {_currency_sym} in Töpfen" if _sp_einz > 0 else ""
                            if _effektiv < _goal_dash:
                                _fehl = _goal_dash - _effektiv
                                st.markdown(
                                    ...
                                    f"<div style='font-family:DM Sans,sans-serif;color:#64748b;font-size:13px;'>Ziel: {_goal_dash:,.2f} {_currency_sym} · Gespart: {_effektiv:,.2f} {_currency_sym}{_th}</div>"
                                    f"</div></div>", unsafe_allow_html=True)
                            else:
                                _ueber = _effektiv - _goal_dash
                                _th2   = f" · {_sp_einz:,.2f} {_currency_sym} davon in Töpfen" if _sp_einz > 0 else ""
                                st.markdown(
                                    ...
                                    f"<div style='font-family:DM Sans,sans-serif;color:#64748b;font-size:13px;'>Gespart: {_effektiv:,.2f} {_currency_sym}{_th2}</div>"
                                    f"</div></div>", unsafe_allow_html=True)
                    ...
                    dep_html  = (f"<div style='flex:1;min-width:160px;background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(56,189,248,0.15);border-radius:16px;padding:20px 22px;'>"
                                 ...
                                 f"<div style='font-family:DM Mono,monospace;font-size:10px;color:#1e293b;margin-bottom:10px;'>diesen Monat eingezahlt</div>"
                                 f"<div style='font-family:DM Sans,sans-serif;color:#38bdf8;font-size:24px;font-weight:600;letter-spacing:-0.5px;'>{dep_monat:,.2f} {_currency_sym}</div></div>") if dep_monat > 0 else ""
                    topf_html = (f"<div style='flex:1;min-width:160px;background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(167,139,250,0.2);border-radius:16px;padding:20px 22px;'>"
                                 ...
                                 f"<div style='font-family:DM Mono,monospace;font-size:10px;color:#1e293b;margin-bottom:10px;'>diesen Monat eingelegt</div>"
                                 f"<div style='font-family:DM Sans,sans-serif;color:#a78bfa;font-size:24px;font-weight:600;letter-spacing:-0.5px;'>{_sp_einz2:,.2f} {_currency_sym}</div></div>") if _sp_einz2 > 0 else ""
                    ...
                    st.markdown(
                        ...
                        f"<div style='font-family:DM Sans,sans-serif;color:{bank_color};font-size:24px;font-weight:600;letter-spacing:-0.5px;'>{bank_str}</div></div>"
                        ...
                        f"<div style='font-family:DM Sans,sans-serif;color:{nw_color};font-size:24px;font-weight:600;letter-spacing:-0.5px;'>{nw_str}</div></div></div>"
                        ...
                        f"<div style='font-family:DM Sans,sans-serif;color:#4ade80;font-size:24px;font-weight:600;letter-spacing:-0.5px;'>+{ein:,.2f} {_currency_sym}</div></div>"
                        f"<div style='flex:1;min-width:160px;background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(148,163,184,0.08);border-radius:16px;padding:20px 22px;'>"
                        ...
                        f"<div style='font-family:DM Sans,sans-serif;color:#f87171;font-size:24px;font-weight:600;letter-spacing:-0.5px;'>-{aus:,.2f} {_currency_sym}</div></div>"
                        + dep_html + topf_html + f"</div>",
                        unsafe_allow_html=True)
                    ...
                    fig.update_layout(
                        ...
                        showlegend=False, margin=dict(t=20,b=20,l=20,r=20), height=380, autosize=True,
                        hoverlabel=dict(bgcolor="#0d1729", bordercolor="rgba(56,189,248,0.4)",
                        ...
                        annotations=[
                            ...
                            dict(text=f"+{ein:,.0f}  /  -{aus:,.0f} {_currency_sym}", x=0.5, y=0.38, showarrow=False,
                    ...
                    with legend_col:
                        ...
                        if sel_cat and sel_typ:
                            ...
                            rows_html = "".join(
                                ...
                                f"<span style='font-family:DM Mono,monospace;color:#64748b;font-size:12px;flex-shrink:0;'>{tr['datum_dt'].strftime('%d.%m.')}</span>"
                                + (f"<span style='color:#94a3b8;font-size:13px;font-family:DM Sans,sans-serif;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;'>{str(tr.get('notiz',''))}</span>"
                                   if str(tr.get('notiz','')).lower() not in ('nan','') else "")
                                + f"</div><span style='color:{sel_color};font-weight:600;font-size:13px;font-family:DM Mono,monospace;flex-shrink:0;margin-left:12px;'>{sign}{tr['betrag_num']:,.2f} {_currency_sym}</span></div>"
                            ...
                            if st.button("← Alle Kategorien", key="dash_back_btn"):
                                ...
                                st.rerun()
                            st.markdown(
                                f"<div style='background:linear-gradient(145deg,rgba(13,23,41,0.95),rgba(10,16,30,0.98));border:1px solid {sel_color}30;border-top:2px solid {sel_color};border-radius:14px;padding:18px 20px;margin-top:10px;box-shadow:0 8px 30px rgba(0,0,0,0.3);'>"
                                f"<div style='font-family:DM Mono,monospace;color:{sel_color};font-size:9px;font-weight:500;letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;'>{sel_typ}</div>"
                                f"<div style='font-family:DM Sans,sans-serif;color:#e2e8f0;font-weight:600;font-size:16px;margin-bottom:4px;letter-spacing:-0.3px;'>{sel_cat}</div>"
                                f"<div style='font-family:DM Mono,monospace;color:{sel_color};font-size:22px;font-weight:500;margin-bottom:18px;letter-spacing:-0.5px;'>{sign}{total_d:,.2f} {_currency_sym}</div>"
                            ...
                            for cat,val,color,typ in zip(all_cats,all_vals,all_colors,all_types):
                                ...
                                with col_legend:
                                    st.markdown(
                                        f"<div style='display:flex;align-items:center;justify-content:space-between;padding:7px 8px;border-radius:8px;border:1px solid transparent;cursor:pointer;'>"
                                        ...
                                        f"<span style='font-family:DM Mono,monospace;color:#334155;font-size:11px;min-width:28px;text-align:right;'>{val/total_sum*100:.0f}%</span>"
                                        f"</div></div>", unsafe_allow_html=True)
                                with col_btn:
    ...
    # ============================================================
        ...
        st.markdown(
            ...
            "<h1 style='font-family:DM Sans,sans-serif;font-size:28px;font-weight:600;color:#e2e8f0;margin:0 0 4px 0;letter-spacing:-0.5px;'>Neue Buchung</h1>"
            "<p style='font-family:DM Sans,sans-serif;color:#334155;font-size:14px;margin:0;'>Erfasse Einnahmen und Ausgaben</p></div>",
            unsafe_allow_html=True)
        ...
        for label, val, col in [("↗ Ausgabe","Ausgabe",ta),("↙ Einnahme","Einnahme",te),("📦 Depot","Depot",td)]:
            with col:
                if st.button(label+(" ✓" if t_type==val else ""), key=f"btn_{val.lower()}",
                             use_container_width=True, type="primary" if t_type==val else "secondary"):
                    st.session_state['t_type'] = val; st.rerun()
        ...
        with st.form("t_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Speichern", use_container_width=True):
                ...
                new_row = pd.DataFrame([{
                    ...
                    "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
                    "typ": t_type, "kategorie": t_cat, "betrag": betrag_save, "notiz": t_note,
                }])
                    ...
                    for cat in custom_cats:
                        ...
                        with cc2:
                            with st.popover("⋯", use_container_width=True):
                                if st.button("✏️ Bearbeiten", key=f"editcat_btn_{cat}", use_container_width=True):
                                    st.session_state.update({'edit_cat_data':{'user':user_name,'typ':t_type,'old_label':cat},'_dialog_just_opened':True})
                                    st.rerun()
                                if st.button("🗑️ Löschen", key=f"delcat_btn_{cat}", use_container_width=True):
                                    st.session_state.update({'delete_cat_data':{'user':user_name,'typ':t_type,'label':cat},'_dialog_just_opened':True})
                                    st.rerun()

        st.markdown("<div style='height:8px'></div><div style='font-family:DM Mono,monospace;color:#334155;font-size:10px;font-weight:500;letter-spacing:1.5px;text-transform:uppercase;margin:20px 0 16px 0;'>Meine Buchungen</div>", unsafe_allow_html=True)
                            ...
                            with st.form(key=f"edit_form_{orig_idx}"):
                                ...
                                with ec2:
                                    e_typ = st.selectbox("Typ", ["Einnahme","Ausgabe","Depot"],
                                                         ...
                                                         if row['typ'] in ["Einnahme","Ausgabe","Depot"] else 1)
                                ...
                                col_save, col_cancel = st.columns(2)
                                with col_save:    saved     = st.form_submit_button("Speichern",  use_container_width=True, type="primary")
                                with col_cancel:  cancelled = st.form_submit_button("Abbrechen",  use_container_width=True)
                                if saved:
                                    df_all = _gs_read("transactions")
    ...
    # ============================================================
        ...
        if df_raw.empty or 'user' not in df_raw.columns:
            ...
            if df_all.empty:
                ...
                zt_col1, zt_col2, zt_col3, _ = st.columns([1,1,1,3])
                for (label, key), col in zip([("Wöchentlich","zt_weekly"),("Monatlich","zt_monthly"),("Jährlich","zt_yearly")],
                                             [zt_col1, zt_col2, zt_col3]):
                    with col:
                        if st.button(label+(" ✓" if zeitraum==label else ""), key=key,
                                     use_container_width=True, type="primary" if zeitraum==label else "secondary"):
                            st.session_state['analysen_zeitraum'] = label; st.rerun()
                ...
                if zeitraum == "Wöchentlich":
                    ...
                    period_mask  = (df_all['datum_dt'].dt.year==now.year) & (df_all['datum_dt'].dt.month==now.month)
                ...
                def make_donut(grp, palette, label, sign, center_color, key_suffix):
                    ...
                    fig.update_layout(
                        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                        showlegend=False, margin=dict(t=10,b=10,l=10,r=10), height=240, autosize=True, dragmode=False,
                    ...
                    rows = "".join(
                        ...
                        f"<span style='font-family:DM Mono,monospace;color:{col};font-size:11px;font-weight:500;white-space:nowrap;'>{sign}{val:,.2f} {_currency_sym}</span>"
                        f"<span style='font-family:DM Mono,monospace;color:#334155;font-size:11px;min-width:26px;text-align:right;'>{val/total*100:.0f}%</span>"
                        f"</div></div>"
                    ...
                    with pie_col: st.plotly_chart(fig, use_container_width=True, key=f"donut_{key_suffix}_{zeitraum}", config={"displayModeBar":False,"staticPlot":True})
                ...
                header_html = "".join(f"<div style='width:42px;text-align:center;font-family:DM Mono,monospace;font-size:10px;color:#334155;letter-spacing:0.5px;padding-bottom:4px;'>{d}</div>" for d in ['Mo','Di','Mi','Do','Fr','Sa','So'])
                ...
                for day in range(1, days_in_month+1):
                    ...
                    cal_cells += (
                        f"<div title='{day}. {hm_label}: {val:.2f} {_currency_sym}' style='width:42px;height:42px;border-radius:8px;background:{bg};border:{border};display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:default;'>"
                ...
                st.markdown(
                    ...
                    f"<div style='display:flex;gap:6px;margin-bottom:6px;flex-wrap:nowrap;'>{header_html}</div>"
                    ...
                    f"<span style='font-family:DM Mono,monospace;font-size:10px;color:#64748b;'>{max_val:.0f} {_currency_sym}</span>"
                    f"</div></div>", unsafe_allow_html=True)
                ...
                with fc_col_r:
                    ...
                    if goal_fc > 0:
                        ...
                        goal_html = (
                            ...
                            f"background:{'rgba(74,222,128,0.06)' if on_track else 'rgba(248,113,113,0.06)'};"
                            f"border:1px solid {'rgba(74,222,128,0.15)' if on_track else 'rgba(248,113,113,0.15)'};display:flex;align-items:center;gap:10px;'>"
                            f"<span style='font-size:16px;'>{'✅' if on_track else '⚠️'}</span>"
                            f"<div><div style='font-family:DM Sans,sans-serif;color:{'#4ade80' if on_track else '#fca5a5'};font-size:13px;font-weight:500;'>{'Sparziel erreichbar!' if on_track else 'Sparziel in Gefahr'}</div>"
                            f"<div style='font-family:DM Mono,monospace;color:#475569;font-size:11px;'>{'Puffer: +' if diff_goal>=0 else 'Fehlbetrag: '}{abs(diff_goal):,.2f} {_currency_sym} vs. Ziel {goal_fc:,.2f} {_currency_sym}</div>"
                            f"</div></div>")
                    st.markdown(
                        f"<div style='background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(148,163,184,0.08);border-radius:16px;padding:20px 22px;'>"
                ...
                if not hist_df.empty and not curr_month_df.empty:
                    ...
                    potenzial_rows = sorted([
                        {'kategorie':kat,'aktuell':curr_val,'durchschn':avg_per_kat.get(kat,0),
                         'diff_pct':(curr_val-avg_per_kat.get(kat,0))/avg_per_kat.get(kat,0)*100,
                         'diff_eur': curr_val-avg_per_kat.get(kat,0)}
                    ...
                    if potenzial_rows:
                        ...
                        st.markdown(
                            ...
                            f"<div style='font-family:DM Sans,sans-serif;color:#94a3b8;font-size:14px;margin-bottom:16px;'>Diesen Monat hast du in <b style='color:#e2e8f0;'>{len(potenzial_rows)} {'Kategorie' if len(potenzial_rows)==1 else 'Kategorien'}</b> mehr ausgegeben als im Durchschnitt. Spar-Potenzial: <span style='color:#4ade80;font-weight:600;'>{total_potenzial:,.2f} {_currency_sym}</span></div>",
                            unsafe_allow_html=True)
                    ...
                    st.markdown("<div style='color:#334155;font-family:DM Sans,sans-serif;font-size:14px;padding:16px;'>Zu wenig Daten für Vergleich. Buche mehrere Monate, um Spar-Potenziale zu sehen.</div>", unsafe_allow_html=True)
                ...
                with sg_col_l:
                    with st.form("sparziel_form"):
                        goal_input = st.number_input(f"Monatliches Sparziel ({_currency_sym})", min_value=0.0,
                ...
                with sg_col_r:
                    if current_goal > 0:
                        ...
                        if not erreicht and fehlbetrag > 0:
                            ...
                            if not kat_monat.empty:
                                ...
                                for _, kr in kat_monat.iterrows():
                                    ...
                                    rows_html += (
                                        f"<div style='display:flex;justify-content:space-between;align-items:center;padding:9px 0;border-bottom:1px solid rgba(255,255,255,0.04);'>"
                                        f"<span style='font-family:DM Sans,sans-serif;color:#94a3b8;font-size:13px;'>{kr['kategorie']}</span>"
                                        f"<div style='display:flex;align-items:center;gap:10px;'>"
                                        f"<span style='font-family:DM Mono,monospace;color:#f87171;font-size:12px;'>−{cut:,.2f} {_currency_sym}</span>"
                                        f"<span style='font-family:DM Mono,monospace;color:#334155;font-size:11px;'>({cut/kr['betrag_num']*100:.0f}%)</span>"
                                        f"</div></div>")
                                    remaining -= cut
                                st.markdown(
                                    f"<div style='background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(248,113,113,0.1);border-left:3px solid #f87171;border-radius:16px;padding:18px 20px;'>"
                            ...
                            st.markdown(
                                f"<div style='background:linear-gradient(145deg,rgba(14,22,38,0.9),rgba(10,16,30,0.95));border:1px solid rgba(74,222,128,0.15);border-left:3px solid #4ade80;border-radius:16px;padding:18px 20px;'>"
                        ...
                        st.markdown(
                            "<div style='background:rgba(15,23,42,0.5);border:1px solid rgba(148,163,184,0.06);border-radius:12px;padding:20px 22px;color:#334155;font-family:DM Sans,sans-serif;font-size:14px;'>Trage links ein Sparziel ein, um Empfehlungen zu erhalten.</div>",
                            unsafe_allow_html=True)
    ...
    # ============================================================
                ...
                with col:
                    if ziel > 0:
                        ...
                        ziel_html= (
                            ...
                            f"<span style='font-family:DM Mono,monospace;color:#334155;font-size:10px;'>{gespart:,.2f} {_currency_sym} von {ziel:,.2f} {_currency_sym}</span>"
                        ...
                        badge_html = (
                            ...
                            if pct>=100 else
                            f"<span style='font-family:DM Mono,monospace;color:#334155;font-size:10px;'>noch {ziel-gespart:,.2f} {_currency_sym} fehlen</span>")
                    else:
                        ziel_html  = ""
                    ...
                    st.markdown(
                        ...
                        f"<div style='font-family:DM Sans,sans-serif;color:#e2e8f0;font-weight:600;font-size:16px;letter-spacing:-0.3px;'>{topf['name']}</div></div>"
                    ...
                    with st.expander(f"💰 Ein/Auszahlen — {topf['name']}"):
                        st.markdown(
                            f"<div style='font-family:DM Sans,sans-serif;color:#475569;font-size:12px;margin-bottom:10px;'>Geld wird als <span style='color:#a78bfa;'>Spartopf-Buchung</span> erfasst und vom Kontostand abgezogen — zählt aber als gespartes Geld.</div>",
                            unsafe_allow_html=True)
        ...
        with st.form("neuer_topf_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Topf erstellen", use_container_width=True, type="primary"):
                if not nt_name.strip():
                    ...
                    save_topf(user=user_name, name=nt_name.strip(), ziel=nt_ziel,
        ...
        if st.session_state.get('topf_edit_data'):
            ...
            def topf_edit_dialog():
                ...
                with cs:
                    if st.button("Speichern", use_container_width=True, type="primary"):
                        update_topf_meta(user=user_name, topf_id=t['id'],
    ...
    # ============================================================
        ...
        if active_tab == "Profil":
            ...
            st.markdown(f"<div style='background:rgba(10,16,30,0.5);border:1px solid rgba(148,163,184,0.06);border-radius:10px;padding:12px 16px;display:inline-block;'><span style='font-family:DM Mono,monospace;color:{_t['primary']};font-size:15px;font-weight:500;'>@{user_name}</span></div>", unsafe_allow_html=True)
        ...
        # ── FINANZEN ─────────────────────────────────────────
            ...
            with col_l:
                ...
                with st.form("budget_form"):
                    new_budget = st.number_input(f"Budget ({_currency_sym})", min_value=0.0, value=float(_user_settings.get('budget',0.0)), step=50.0, format="%.2f", help="0 = kein Limit gesetzt")
                ...
                if _user_settings.get('budget',0) > 0:
                    st.markdown(f"<div style='background:rgba(10,16,30,0.5);border:1px solid rgba(148,163,184,0.06);border-radius:10px;padding:12px 16px;margin-top:8px;'><span style='font-family:DM Mono,monospace;color:#64748b;font-size:12px;'>Aktuelles Limit: </span><span style='font-family:DM Mono,monospace;color:{_t['primary']};font-size:13px;font-weight:600;'>{_user_settings['budget']:,.2f} {_currency_sym}</span></div>", unsafe_allow_html=True)
            with col_r:
        ...
        # ── DESIGN ───────────────────────────────────────────
            ...
            for i, tname in enumerate(THEMES.keys()):
                ...
                with theme_cols[i]:
                    _tc_bor = t_data['primary']+'ff' if is_active_theme else t_data['primary']+'30'
                    _tc_sha = f"box-shadow:0 0 20px {t_data['primary']}30;" if is_active_theme else ""
                    _tc_badge = f"<div style='margin-top:8px;font-family:DM Mono,monospace;color:{t_data['primary']};font-size:9px;letter-spacing:1.5px;'>✓ AKTIV</div>" if is_active_theme else ""
                    st.markdown(
                    ...
                    if st.button("Auswählen" if not is_active_theme else "✓ Aktiv",
                    ...
                    with st.form("email_change_form"):
                        ...
                        if st.form_submit_button("Code senden", use_container_width=True, type="primary"):
                            if not is_valid_email(new_email_input.strip()):
                                ...
                                if send_email(new_email_input.strip().lower(), "Balancely – E-Mail-Adresse bestätigen",
                                              email_html("Dein Code zum Ändern der E-Mail-Adresse lautet:", code)):
                    ...
                    with st.form("email_verify_form"):
                        ...
                        if confirm:
                            if st.session_state['email_verify_expiry'] and datetime.datetime.now() > st.session_state['email_verify_expiry']:
                                ...
                                st.session_state['email_verify_code'] = ""; st.rerun()
    ...
    with center_col:
        ...
        if mode == 'login':
                    ...
                    if not all([s_name, s_user, s_email, s_pass]):
                        ...
                        if not ok:
                            ...
                            if s_user in df_u['username'].values:
                                ...
                                code   = generate_code()
                                expiry = datetime.datetime.now() + datetime.timedelta(minutes=10)
                                if send_email(s_email.strip().lower(), "Balancely – E-Mail verifizieren",
                                              email_html("Willkommen bei Balancely! Dein Verifizierungscode lautet:", code)):
            ...
            with st.form("verify_form"):
                st.markdown("<h3 style='text-align:center;color:#e2e8f0;font-family:DM Sans,sans-serif;font-weight:600;font-size:22px;letter-spacing:-0.5px;margin-bottom:12px;'>E-Mail verifizieren</h3>", unsafe_allow_html=True)
                st.markdown(f"<p style='text-align:center;color:#475569;font-family:DM Sans,sans-serif;font-size:14px;margin-bottom:20px;'>Code gesendet an <span style='color:#38bdf8;'>{pending_email}</span></p>", unsafe_allow_html=True)
            ...
            with st.form("forgot_form"):
                st.markdown("<h3 style='text-align:center;color:#e2e8f0;font-family:DM Sans,sans-serif;font-weight:600;font-size:22px;letter-spacing:-0.5px;margin-bottom:12px;'>Passwort vergessen</h3>", unsafe_allow_html=True)
                st.markdown("<p style='text-align:center;color:#475569;font-family:DM Sans,sans-serif;font-size:14px;margin-bottom:20px;'>Gib deine E-Mail-Adresse ein.</p>", unsafe_allow_html=True)
                ...
                if st.form_submit_button("Code senden", use_container_width=True):
                    if not is_valid_email(forgot_email):
                        ...
                        if idx.empty:
                            ...
                            code   = generate_code()
                            expiry = datetime.datetime.now() + datetime.timedelta(minutes=10)
                            if send_email(forgot_email.strip().lower(), "Balancely – Passwort zurücksetzen",
                                          email_html("Dein Code zum Zurücksetzen des Passworts lautet:", code)):
            ...
            with st.form("reset_form"):
                st.markdown("<h3 style='text-align:center;color:#e2e8f0;font-family:DM Sans,sans-serif;font-weight:600;font-size:22px;letter-spacing:-0.5px;margin-bottom:12px;'>Passwort zurücksetzen</h3>", unsafe_allow_html=True)
                st.markdown(f"<p style='text-align:center;color:#475569;font-family:DM Sans,sans-serif;font-size:14px;margin-bottom:20px;'>Code gesendet an <span style='color:#38bdf8;'>{st.session_state['reset_email']}</span></p>", unsafe_allow_html=True)
